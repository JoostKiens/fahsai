...
export function useCamsBitmap(data: PM25GridPoint[] | undefined): ImageBitmap | null {
  const [bitmap, setBitmap] = useState<ImageBitmap | null>(null);
  const workerRef = useRef<Worker | null>(null);

  useEffect(() => {
    const worker = new Worker(new URL('../workers/camsCanvas.worker.ts', import.meta.url), {
      type: 'module',
    });
