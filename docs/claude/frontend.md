...
| PM2.5 stations | `IconLayer` + `ScatterplotLayer` + `TextLayer` (`PM25Layer.ts`)                         | OpenAQ ground stations, Supercluster-grouped; single stations render as 15px-radius dots colored by `pm25ToRgba(d.value)`; clusters render as diamond icons (canvas atlas) with a two-line label (mean value + ×count)                                     |
